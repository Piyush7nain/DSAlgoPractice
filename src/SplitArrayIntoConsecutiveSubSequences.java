...
public class SplitArrayIntoConsecutiveSubSequences {
    ...
    public boolean isPossible(int[] nums) {
        ...
        for(int n : nums) {
            ...
            if(lists.get(n-1) != null && lists.get(n-1) > 0 ){
                lists.put(n, lists.getOrDefault(n, 0) + 1);
                lists.put(n-1, lists.getOrDefault(n-1,0)-1);
            }else if(map.containsKey(n+1) && map.get(n+1) > 0 && map.containsKey(n+2) && map.get(n+2) > 0){
                lists.put(n+2, lists.getOrDefault(n+2, 0) + 1);
                map.put(n+2, map.getOrDefault(n+2, 0) - 1);
