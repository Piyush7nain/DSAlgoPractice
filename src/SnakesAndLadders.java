...
public class SnakesAndLadders {
    ...
    public int snakesAndLadders(int[][] board) {
        simpleBoard = createSimpleBoard(board);
        visited = new int[simpleBoard.length];
        dfs(1, 0);

        return minSteps;
    }

    void dfs(int start, int currSteps) {
        if(currSteps>simpleBoard.length*simpleBoard.length){
            return;
        }
        System.out.println("Reached : "+ start +" on step: "+currSteps);
        /*try {
            Thread.sleep(500);
        } catch (InterruptedException e) {
            throw new RuntimeException(e);
        }*/
        if(start == simpleBoard.length - 1) {
        ...
        int s = start +1;
        int e = Math.min(start + 6, simpleBoard.length - 1);
        visited[start] =1;
        for(int i = s; i <=e; i++) {
            int nextStep = i;
            if(simpleBoard[nextStep] != -1) {
                nextStep = simpleBoard[nextStep];
            }

            dfs(nextStep, currSteps + 1);

        }
        visited[start] = 0;

    }
    List<Integer> getNextSteps(int start) {
        int s = start +1;
        int e = Math.min(start + 6, simpleBoard.length - 1);
    ...
    int[] createSimpleBoard(int[][] board) {
        int m = board.length;
        int counter = 1;
        int pointer = 0;
        int direction = 1;
...
//        int[][] board = {{-1,10,-1,15,-1},{-1,-1,18,2,20},{-1,-1,12,-1,-1},{2,4,11,18,8},{-1,-1,-1,-1,-1}};
        int[] indexs = new int[board.length*board.length+1];
        for(int i = 0; i < board.length*board.length+1; i++) {
            indexs[i] = -i;
        }
